...
public class EvilWizard extends Gameobject implements Mob {
	...
	public void tick(){
		if(x>p.x){
			...
			if(new Random().nextInt(40)==0){
				...
				if(new Random().nextBoolean()==true){
					if(direction==1){
					hand.NewObject(new FireBall(x+width, y+width/2, hand, FireBall.SPEED*direction, (p.y+(p.height/2))-this.y/(p.x-this.x/30)));
				}else{
					hand.NewObject(new FireBall(x-width, y+width/2, hand, FireBall.SPEED*direction,(p.y+(p.height/2))-this.y/(p.x-this.x/30)));
				}
				}else{
					if(direction==1){
					hand.NewObject(new ElStrike(x+width, y+width/2, hand, ElStrike.SPEED*direction,(p.y+(p.height/2))-this.y/(p.x-this.x/50)));
				}else{
					hand.NewObject(new ElStrike(x-width, y+width/2, hand, ElStrike.SPEED*direction,(p.y+(p.height/2))-this.y/(p.x-this.x/50)));
				}
				}
		...
		if(!jumping&&new Random().nextInt(10)==0){
			vely-=20;
			jumping=true;
		}
